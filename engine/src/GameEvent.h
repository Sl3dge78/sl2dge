...
namespace sl2dge {
	...
	public:
		...
		void resize(int x, int y) {
			rect_.w += x; if (rect_.w < 100) rect_.w = 100;
			rect_.h += y; if (rect_.h < 100) rect_.h = 100;
			on_box_moved();
		}
		void translate(int x, int y) { rect_.x += x; rect_.y += y; on_box_moved(); }
	...
	protected:
		 GameEvent(const Guid id = newGuid()) :id_(id) {};
