...
namespace sl2dge {
	...
	public:
		DialogState(EventChain* event_chain, Dialog* dialog) : current_dialog(dialog), event_chain(event_chain){};
		DialogState(EventChain* event_chain, DialogChoice* dialog) : current_dialog(dialog), current_choice_dialog(dialog), event_chain(event_chain) {
			is_choice = true;
		};
		~DialogState() {
